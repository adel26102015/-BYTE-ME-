...
from data.data import cold_drinks_items
import time




st.set_page_config(page_title="Cold_Drinks", layout="wide")

st.title("COLD DRINKS")
...
if st.session_state.is_signed_in == False:
    ...
    for i, item in enumerate(cold_drinks):
        ...
        image = item["image"]
        with cols[i]:
            with st.container(key=f"cold_drinks_form_{item}"):
                st.subheader(name)
                st.subheader(price)            
