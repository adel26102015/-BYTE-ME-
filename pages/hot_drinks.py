...
from data.data import hot_drinks_items
import time 

st.logo("images/LOGO.jpg")
        

st.set_page_config(page_title="HOT DRINKS", layout="wide")
...
if st.session_state.is_signed_in == False:
    ...
    for i, item in enumerate(hot_drinks):
        name = item["name"]
        ...
        image = item["image"]
        with cols[i]:
            with st.container(key=f"hot_drinks_form_{item}"):
                st.subheader(name)
                st.subheader(price)            
