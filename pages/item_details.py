...
st.session_state.selected_item = st.session_state.selected_item if "selected_item" in st.session_state else None
    ...
    with c2 :
        ...
        with col1:
            if st.button("➕", key=f"{item}_add_btn", use_container_width=True):
                st.session_state.orders_list.append({"name": item_name, "price":item_price})
        with col2:
            if st.button("➖", key=f"{item}_remove_btn", use_container_width=True):
                st.session_state.orders_list.remove({"name": item_name, "price":item_price})   
        st.divider()
